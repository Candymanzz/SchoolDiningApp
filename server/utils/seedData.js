    ...
    try {
        ...
        for (let i = 0; i < 50; i++) {
            const classData = await Class.create({
                name: `${Math.floor(Math.random() * 11) + 1}${String.fromCharCode(65 + Math.floor(Math.random() * 3))}`,
        ...
        for (let i = 0; i < 50; i++) {
            await Participant.create({
                ...
                studentStudentId: students[Math.floor(Math.random() * students.length)].student_id,
                nutritionNutritionId: nutritions[Math.floor(Math.random() * nutritions.length)].nutrition_id,
                classClassId: classes[Math.floor(Math.random() * classes.length)].class_id
            });
        }
